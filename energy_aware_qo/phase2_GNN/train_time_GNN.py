...
# 1. DATASET EXTRACTION AND ASSEMBLING

CSV_PATH = "../datasets/tpch_multi_scale_final_dataset.csv"
df = pd.read_csv(CSV_PATH)
graph_dataset = []

print("Assembling Query Graphs with Scale Identity Integration")
for idx, row in df.iterrows():
    ...
    if os.path.exists(json_path):
        ...
        # Handing down 5 arguments including query identity tracking
...
optimizer = torch.optim.Adam(model_time.parameters(), lr=0.001)
...
# 2. Training loop with LOG target & MAPE tracking
print("\n Training Upgraded GNN Time Model (Log-Stabilized Target Space)")
for epoch in range(350):
    ...
    for batch in loader:
        ...
        log_predictions = model_time(batch)
        
        # Compress target values into log-space to balance gradients
        log_time_targets = torch.log1p(batch.y_time)
        
        # Compute loss natively in the compressed log space
        loss = loss_fn(log_predictions, log_time_targets)
        loss.backward()
        ...
        # Converts log-space prediction back to real-world scale (seconds or Joules)
        true_scale_predictions = torch.expm1(log_predictions)
        total_mape += compute_mape(true_scale_predictions, batch.y_time) * batch.num_graphs
...
# 3. SAVE WEIGHTS & LOGS
torch.save(model_time.state_dict(), "gnn_time_model.pth")
pd.DataFrame(training_history_log).to_csv("gnn_training_loss_history.csv", index=False)
print("\n Model trained and weights saved to gnn_time_model.pth")